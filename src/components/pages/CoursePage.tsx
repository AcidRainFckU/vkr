...
const CoursePage: FC<Props> = ({ course }) => {
  ...
  const [filteredArr, newFilteredArr] = useState<Course[]>([]);

  const [chapterCreate, setChapterCreate] = useState({
    title: '',
    courseId: courseId,
  });
  const currentCourse = course?.filter((el) => el.id.toString() === courseId)[0];
  ...
  useEffect(() => {
    ...
    const arr = sortByAge(currentCourse?.chapters);
    newFilteredArr(arr);
  }, [currentCourse]);
  ...
  async function handleCreateChapter(e: any) {
    e.preventDefault();
    await CoruseService.createChapter(chapterCreate.title, Number(chapterCreate.courseId));
    alert('Раздел успешно создан');
  }

  return (
    <PageLayout>
      <h2 className=" text-3xl font-bold text-gray-500 mb-5">{currentCourse.title}</h2>
      {user?.role === 'superuser' && (
        <button
      ...
      {/* {user?.role === 'superuser' && (
        <button
          className="my-7 text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-xl px-5 py-2.5 text-centermb-7  "
          onClick={() => dispatch(dropCourseAction(user.id, courseId))}>
          Удалить курс
        </button>
      )} */}
      <div
        ...
        <div className="course__users drop-shadow-md bg-white rounded-xl p-8 relative flex flex-col w-96">
          ...
          <form className="flex flex-col" onSubmit={(e) => handleCreateChapter(e)}>
            <input
              ...
              onChange={(e) => setChapterCreate({ ...chapterCreate, title: e.target.value })}
            />
      ...
      <div className="w-full h-full shadow-md bg-white p-7 " id="sidenavExample">
